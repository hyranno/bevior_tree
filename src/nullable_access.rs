...
use bevy::{
    ecs::{component::Tick, system::SystemState},
    prelude::*,
};

use crate::{task::TaskState, NodeResult};
...
impl NullableWorldAccess {
    ...
    /// Read and write access to the world
    fn call_system_sys<In: 'static, Out: 'static>(
        &mut self,
        input: In,
        sys: &mut Box<dyn System<In = In, Out = Out>>,
    ) -> Result<Out, NullableAccessError> {
        match self.ptr.as_deref_mut() {
            Some(world) => {
                // While `System` does not have `is_initialized` thing, use `last_run` to check if it is initialized.
    ...
    pub fn check_task(
        &mut self,
        entity: Entity,
        checker: &mut Box<dyn System<In = Entity, Out = TaskState>>,
    ) -> Result<TaskState, NullableAccessError> {
        self.call_system_sys(entity, checker)
    }
    ...
    pub fn entity_command_call(
        &mut self,
        entity: Entity,
        system: &(impl Fn(Entity, Commands) + Send + Sync),
