...
use super::{nullable_access::NullableWorldAccess, Node, NodeGen, NodeResult, ResumeSignal};
...
pub struct TaskImpl {
    checker: Mutex<Box<dyn System<In = Entity, Out = TaskState>>>,
...
impl TaskImpl {
    ...
    pub fn on_enter(mut self, callback: impl 'static + Fn(Entity, Commands) + Send + Sync) -> Self {
        self.on_enter.push(Box::new(callback));
        self
    }
    pub fn on_exit(mut self, callback: impl 'static + Fn(Entity, Commands) + Send + Sync) -> Self {
    ...
    pub fn insert_while_running<T: Bundle + 'static + Clone>(self, bundle: T) -> Self {
        self.on_enter(Box::new(move |entity, mut commands: Commands| {
    ...
    fn check(&self, world: Arc<Mutex<NullableWorldAccess>>, entity: Entity) -> TaskState {
        world
            .lock()
            .unwrap()
            .check_task(entity, self.checker.lock().as_deref_mut().unwrap())
            .unwrap()
    }
    fn trigger_enter(&self, world: Arc<Mutex<NullableWorldAccess>>, entity: Entity) {
        for event in self.on_enter.iter() {
            world
                .lock()
                .unwrap()
                .entity_command_call(entity, &event)
                .unwrap();
        }
    }
